...
public class SystemService extends BaseService implements InitializingBean {
	...
	public int saveUser(User user) {
		if (StringUtils.isBlank(user.getId())){
			...
			User users = userDao.getUserByName(name);
			if (users == null) {
				userDao.insert(user);
				return 1;
			} else {
...
//		systemRealm.clearAllCachedAuthorizationInfo();
	 ...
	 */
	@Transactional(readOnly = false)
	public boolean autoInsertUser(User user) {
		try {
			String currentUser = UserUtils.getUser().getId();//当前登录的用户
			user.setId(IdGen.uuid());
			user.setCreateAById(currentUser); // 新增对象
			if(userDao.autoInsertUser(user)>0) {
				return true;
			}
		} catch (Exception e) {
			logger.error("SystemService.autoInsertUser error:",e);
		}
		return false;
	}
	...
	public static void main(String[] args) {
		String s = "24903cc776432f30a7c4bd487c03c3672847ed8154fdac791577a177";
		System.out.println(validatePassword("111111",s));
	}
}
