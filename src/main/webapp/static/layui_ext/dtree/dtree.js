...
layui.define(['jquery','layer','element','form'], function(exports) {
	...
	DTree.prototype.openAllNode = function(obj){	
		...
		console.log("首次进来"+obj);
		
		var _this = this;
	
		var $ulNode = obj || _this.obj.children("li").children("ul");
		
		console.log("当前："+$ulNode.length);
	...
	DTree.prototype.bindBrowserEvent = function(nodes){
		...
		_this.obj.on("click", "i[data-spread]", function(event) {
			...
			// 获取当前图标获取的CODE属性值
			// 将当前选定的CODE值返回到自定义的DtreeNodeCode返回隐藏域中
			$("#dtreeNodeCode").val(node.nodeId);
			// 获取父ID
			$("#parentId").val(node.parentId);
			// 节点ID
			$('#parentIdInput').val(node.nodeId);
			// 节点name
			$('#parentNameInput').val(node.context);
			// 节点路径
			$('#catPathInput').val(node.parentId);
			// 节点级别
			$('#catLevelTree').val(node.level);
		});
