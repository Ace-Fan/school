...
<body>
	...
	<div class="layui-main">
	  	...
	  	<fieldset class="layui-elem-field layui-field-title">
		    <legend>
		    	<c:choose>
					...
					<c:otherwise>
		        		司机提现信息${wmDriverWallet.id eq null ?'添加':'修改'}
		        	</c:otherwise>
				</c:choose>
			</legend>
		</fieldset>
	    ...
	    <form class="layui-form layui-form-pane" action="" style="margin-top: 1em;">
			<div id="dis">
				<input type="hidden" name="id" value="${wmDriverWithdraw.id}" />
				<input type="hidden" name="userId" id="userId" value="${wmDriverWithdraw.userId}" />
						...
						<div class="layui-input-inline">
							<select lay-filter="userId" lay-verify="required" autocomplete="off" lay-search>
								...
								<c:forEach var="item" items="${uwList}">
									<option ${wmDriverWithdraw.userId eq item.userId ? 'selected':'' } value="${item.userId}-${item.balance}-${item.userName}">${item.userName}</option>
								</c:forEach>
							</select>
						</div>
					</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<input type="text" id="userName" placeholder="请选择司机联系电话" readonly="readonly"
								autocomplete="off" class="layui-input" value="${wmDriverWithdraw.userName}" maxlength="25">
						</div>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						<label class="layui-form-label">司机余额</label>
						<div class="layui-input-inline">
							<input type="text" id="userBalance" placeholder="请选择司机联系电话" readonly="readonly"
								autocomplete="off" class="layui-input" value="${wmDriverWithdraw.userBalance}" maxlength="25">
						</div>
					</div>
				</div>
					...
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<input type="text" name="withdrawAmount" lay-verify="required" placeholder="请输入当前提现" oninput ="value=value.replace(/[^0-9.]/g,'')"
								autocomplete="off" class="layui-input" value="${wmDriverWithdraw.withdrawAmount}" maxlength="25">
						</div>
					</div>
				</div>
				...
				<!-- 认证状态 -->
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<c:choose>
								<c:when test="${wmDriverWithdraw.status eq null}">
								...
								<c:otherwise>
									<input type="radio" name="status" ${wmDriverWithdraw.status eq '1'?'checked':''} value="1" title="审核中"  />
									<input type="radio" name="status" ${wmDriverWithdraw.status eq '2'?'checked':''} value="2" title="提现中" />
									<input type="radio" name="status" ${wmDriverWithdraw.status eq '3'?'checked':''} value="3" title="审核不通过" />
									<input type="radio" name="status" ${wmDriverWithdraw.status eq '4'?'checked':''} value="4" title="提现成功" />
									<input type="radio" name="status" ${wmDriverWithdraw.status eq '5'?'checked':''} value="5" title="提现失败" />
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<input type="radio" name="isEnable" ${wmDriverWithdraw.isEnable eq '0'?'':'checked'} value="1" title="启用" />&ensp;&ensp;  
							<input type="radio" name="isEnable" ${wmDriverWithdraw.isEnable eq '0'?'checked':''} value="0" title="禁用" />
						</div>
					</div>
				</div>
			   ...
			   <div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<textarea placeholder="请输入备注"  maxlength="100" name="auditRemark" class="layui-textarea" >${wmDriverWithdraw.auditRemark}</textarea>
						</div>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<textarea placeholder="请输入备注" maxlength="100" name="remark" class="layui-textarea">${wmDriverWithdraw.remark}</textarea>
						</div>
					</div>
				</div>
