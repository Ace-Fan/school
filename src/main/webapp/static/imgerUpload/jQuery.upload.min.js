(function(a) {
    a.fn.upload = function(D) {
        ...
        r.append(A.attr("data-num", B.num).attr("data-type", B.type)).append(d.attr("multiple", B.num > 1 ? "multiple" : false)).append(b.attr("name", B.inputname)).addClass(B.num > 1 ? "multiple" : "one").on("click", "li.add", function() {
            ...
            z(a(i.currentTarget))
        }).on("click", "svg.delete", function(i) { // 删除触发时间
        	z(a(i.currentTarget).parent("li.success"))
        }).on("click", "svg.look", function(i) { // 查看触发事件
                    ...
                    var i = l(G[F], function(K, J, I) {
                        ...
                        a.ajax({
                            ...
                            success: function(M) {
                                ...
                                // 上传图片成功
                                if (M.code) {
                                	var val =  $("#imgurl").val(); // 获取隐藏的文本域的value
                                    val+=M.result+","; // 对获取到的value进行一个拼接
                                    $("#imgurl").val(val); // 隐藏的文本域赋值拼接好的value
            ...
            for (var F in G) {
                if (typeof G[F] == "object") {
                    var i = l(G[F], function(K, J, I) {
                        ...
                        // 分割.后面的文件名
                        var imgVar = imgForm.split(".");
                        // 文件后缀名
                        $('#imgFormat').val(imgVar[1]);
                        ...
                        a.ajax({
                            ...
                            success: function(M) {
                                ...
                                var valOneFlag = $("#valOneFlag").val();
                                
                                console.log("valOneFlag:"+valOneFlag);
                                
                                // 新增多个单张图片上传JS
                                if (valOneFlag == "1") { // 第一个单张图片回调
                                	var valOne1 = M.result;
                                    $("#valOne1").val(valOne1);
                                } else if (valOneFlag == "2") {	// 第二个单张图片回调
                                	var valOne2 = M.result;
                                    $("#valOne2").val(valOne2);
                                } else if (valOneFlag == "3") {	// 第三个单张图片回调
                                	var valOne3 = M.result;
                                    $("#valOne3").val(valOne3);
                                } else if (valOneFlag == "4") {	// 第四个单张图片回调
                                	var valOne4 = M.result;
                                    $("#valOne4").val(valOne4);
                                } else if (valOneFlag == "5") {	// 第五个单张图片回调
                                	var valOne5 = M.result;
                                    $("#valOne5").val(valOne5);
                                } else if (valOneFlag == "6") {	// 第六个单张图片回调
                                	var valOne6 = M.result;
                                    $("#valOne6").val(valOne6);
                                } else if (valOneFlag == "7") {	// 第七个单张图片回调
                                	var valOne7 = M.result;
                                    $("#valOne7").val(valOne7);
                                } else if (valOneFlag == "8") {	// 第八个单张图片回调
                                	var valOne8 = M.result;
                                    $("#valOne8").val(valOne8);
                                } else if (valOneFlag == "9") {	// 第九个单张图片回调
                                	var valOne9 = M.result;
                                    $("#valOne9").val(valOne9);
                                } else {						// 默认图片回调
                                	var valOne = M.result;
                                    $("#valOne").val(valOne);
                                ...
                                $.ajax({
                                 	url:'/school/a/base/group/getImgConfig',
                                 	type:'POST',
                                 	data:{'url':M.result},
                    ...
                    var i = l(G[F], function(K, J, I) {
                        ...
                        a.ajax({
                            ...
                            success: function(M) {
                                ...
                                if (M.code) {
                                	var val =  $("#valOne2").val(); // 获取隐藏的文本域的value
                                    val+=M.result+","; // 对获取到的value进行一个拼接
                                    $("#valOne2").val(val); // 隐藏的文本域赋值拼接好的value
                                    J.addClass("success").append(g).append(typeof D == "function" ? E : "").data("url", M.msg)
                                } else {
                                    J.addClass("error").attr("data-error", M.msg ? M.msg : "服务端返回数据异常")
                                }
            ...
            H.onload = function() {
                ...
                if (["png", "jpg", "jpeg", "gif", "bmp"].indexOf(J) >= 0) {
                    ...
                    O.onload = function() {
                        ...
                        var U = Q.toDataURL("image/png", Y);//image/jpeg
