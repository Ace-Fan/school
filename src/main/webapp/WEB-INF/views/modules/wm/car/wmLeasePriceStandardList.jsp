...
<body>
		...
		<div class="layui-inline" style="margin-top: 0.5rem;">
			<form id="selectDiv">
				...
				<div class="layui-inline layui-form selectDiv-input">
					<select lay-filter="carSpecId" id="carSpecId">
						<option value="">车厢规格（全部）</option>
						<c:forEach var="item" items="${wmLeaseCarSpecList}">
							<option value="${item.id}">${item.id}-${item.carTypeName}-${item.carLength}</option>
						</c:forEach>
					</select>
				</div>
	...
	<script type="text/javascript">
		layui.use([ 'table', 'form' ], function() {
			...
			form.on('select(carSpecId)', function(data) { if (data.value == '') { reloadTable(); } });						// 监听搜索框下拉框选择为ALL时刷新表单
