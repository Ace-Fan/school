...
<body>
	...
	<div class="layui-main">
	  	...
	  	<fieldset class="layui-elem-field layui-field-title">
		    <legend>
		    	<c:choose>
					...
					<c:otherwise>
		        		用户钱包信息${wmUserWallet.id eq null ?'添加':'修改'}
		        	</c:otherwise>
				</c:choose>
			</legend>
		</fieldset>
	    ...
	    <form class="layui-form layui-form-pane" action="" style="margin-top: 1em;">
			<div id="dis">
				<input type="hidden" name="id" value="${wmUserWallet.id}" />
						...
						<div class="layui-input-inline">
							<select lay-filter="userId" lay-verify="required" id="userId" name="userId" autocomplete="off" lay-search>
								...
								<c:forEach var="item" items="${wmOrderInfoList}">
									<option ${wmUserWallet.userId eq item.id ? 'selected':'' } value="${item.id}">${item.phone}</option>
								</c:forEach>
							</select>
						</div>
					</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<input type="text" name="balance" lay-verify="required" placeholder="请输入钱包余额" oninput ="value=value.replace(/[^0-9.]/g,'')"
								autocomplete="off" class="layui-input" value="${wmUserWallet.balance}" maxlength="25">
						</div>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<input type="radio" name="isEnable" ${wmUserWallet.isEnable eq '0'?'':'checked'} value="1" title="启用" /> 
							<input type="radio" name="isEnable" ${wmUserWallet.isEnable eq '0'?'checked':''} value="0" title="禁用" />
						</div>
					</div>
				</div>
				...
				<div class="layui-form-item">
					<div class="layui-col-xs12">
						...
						<div class="layui-input-inline">
							<textarea placeholder="请输入备注" maxlength="100" name="remark" class="layui-textarea">${wmUserWallet.remark}</textarea>
						</div>
					</div>
				</div>
